...
static long Plugin_143_millisPresenceTime = 10;
...
boolean Plugin_143(byte function, struct EventStruct *event, String& string)
{
  ...
  switch (function)
  {
    ...
    case PLUGIN_WEBFORM_LOAD:
      {
        ...
        string += F("<TR><TD>Presence Time:<TD>");
        addNumericBox(string, F("ptime"), Settings.TaskDevicePluginConfig[event->TaskIndex][0]);
        string += F(" [sec]");
    ...
    case PLUGIN_WEBFORM_SAVE:
      {
        String plugin1 = WebServer.arg(F("ptime"));
        Settings.TaskDevicePluginConfig[event->TaskIndex][0] = plugin1.toInt();

        Settings.TaskDevicePluginConfig[event->TaskIndex][1] = (WebServer.arg(F("useain")) == "on");
    ...
    //case PLUGIN_TEN_PER_SECOND:
      {
        ...
        {
          if (Plugin_143_millisPresenceEnd < millis())   // timeout?
          {
            send = true;
